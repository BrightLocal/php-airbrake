...
class EventHandler {
     ...
     * @return EventHandler
     */
    public static function start($apiKey, $notifyOnWarning = false, array $options = []) {
        if (!isset(self::$instance)) {
            self::$instance = new self(new Client(new Configuration($apiKey, $options)), $notifyOnWarning);
            set_error_handler([self::$instance, 'onError']);
            set_exception_handler([self::$instance, 'onException']);
            register_shutdown_function([self::$instance, 'onShutdown']);
        }
        return self::$instance;
    }
